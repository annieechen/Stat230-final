...
* `Various Subject Names (TT2G15[A - L]):` 12 columns representing the subjects teachers teach (boolean numeric variable)
...
p2 <- teacher_data %>% filter(!is.na(collab)) %>% ggplot(aes(x=collab, y=sat_avg)) + geom_boxplot() +
  labs(title="Relationship Between Collaboration Level
       and Teacher Satisfaction",
       x="Collaboration Level of the School",
       y="Teacher Satisfaction")

# Boxplot of region to satisfaction
p3 <- teacher_data %>% filter(!is.na(sat_avg)) %>% ggplot(aes(x=reorder(region, -sat_avg, median), y=sat_avg)) + geom_boxplot() + labs(title="Teacher Satisfaction by Region", x="Region", y="Teacher Satisfaction")
```
