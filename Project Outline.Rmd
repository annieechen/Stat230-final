...
We'll split the country data into regions using the United Nations Geoscheme categories, so that we can try to find regional patterns
...
* `region:` Region the teacher's country is in (factor variable, 5 levels)
...
* `avg_sat:` An average of the ten questions related to teacher job satisfaction (numeric variable)
...
```{r}
teacher_data %>% ggplot(aes(x=age, y=years_teacher, color=region)) + geom_point(alpha= 0.5, shape=".") + geom_smooth(method="lm", se=F)
```
...
#### One-way Anova Tests
Based on results so far, some potential candidates for predictors are: 
