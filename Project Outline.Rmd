...
p2 <- teacher_data %>% filter(!is.na(collab)) %>% ggplot(aes(x=collab, y=sat_avg)) + geom_boxplot() + xlab("Collaboration") + ylab("Teacher Satisfaction")

# Boxplot of region to satisfaction
p3 <- teacher_data %>% filter(!is.na(sat_avg)) %>% ggplot(aes(x=reorder(region, -sat_avg, median), y=sat_avg)) + geom_boxplot() + xlab("Region") + ylab("Teacher Satisfaction")
```
...
Plot 3 provides a box plot of geographic region vs sat_avg. Our second research question revolves around variations in sat_avg and other variables across different geographical regions. Visually, while all median values for sat_avg hover around the range of 2.5 to 3.5, there is still a noticeable difference in some of the distributions (e.g. the median and both quartiles of the satisfaction value for North America is noticeably above that of Eastern Europe). We will explore additional plots like this one in our analysis below, with other variables and their relationship to region.
...
To address our second research question, and try to identify the differences across different regions for teacher job satisfaction and other classroom related variables, we are going to try several techniques.
...
(2) We are going to plot each of the predictive factors that were found to be significant in Research Question 1 against region, to see if perhaps region is correlated with some of these other variables.  It's possible that the differences in avg_sat by region could be due to other variables that are correlated with region.
(3) Additionally, for the variables that were significant in part (2), we will also graph sat_avg as a function of that variable, but using separate graphs for each region (using faceting). This is to see if the relationship between that variable and sat_avg changes between different region.

#### One-way ANOVA Test

After the analysis above is completed, we'll run ANOVA tests to see if there are statistically significant differences in mean `sat_avg` across regions.
...
Finally, we will make two linear models. The first linear model will try to predict sat_avg using only the region variable.  The second linear model will try to predict sat_avg using the region variable and all of the variables that seemed correlated with region in the analysis above.  We will then compare these two models to the best model found in part (1), in order to see if region or variables correlated with region heavily contributed to the success of the model in part 1, or if other predictors were more important.
