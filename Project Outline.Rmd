...
To address our first research question, and to try to identify which variables are the best indicators of teacher job satisfaction, we are going to attempt to several techniques.
...
(2) We are going to use forward, backward and bidirectional stepwise selection, and to look at the linear models that end up getting chosen from those three methods.
...
Then we will analyze the models from (1) and (2) to try to determine which columns are the best predictors of whether or not teachers are satisfied, and to compare these results to the results we got from linear regression.
