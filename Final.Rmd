...
x.new %>% ggplot(aes(x=sat_avg^2)) + geom_histogram(binwidth = 1) + labs(title="Histogram of Teacher Satisfaction Squared", x="Teacher Satisfaction Squared (low to high)", y="Count")
```
We can see from the two histograms above, that the histogram of sat_avg looks a little bit closer to a normal distribution if we square it.
...
x.new %>% ggplot(aes(x=log(years_teacher))) + geom_histogram() + labs(title="Histogram of Years Spent as a Teacher", x="Years of Teaching Experience", y="Count")
```
We can see from the histograms of years spent teaching that the distribution has many values clumped on the lower end of the x values.  The histogram of the log transformation of years spent teaching, however, also does not look like a normal distribution.  However, it might be worth trying in the linear model.  Let's see if squareing sat_avg, or taking the log transform of years_teacher improves the model.

```{r}
m1 <- lm(I(sat_avg^2) ~ ., data=x.new)
summary(m1)
```

Squaring our response variable did cause a slight improvement in R-squared, from 0.2449 to 0.2464.
QUESTION: is this enough of an improvement to keep the transformations?

Now let's see if taking the log of years_teacher helps.

```{r}
m2 <- lm(I(sat_avg^2) ~ country + gender + age + log(years_teacher + 0.0001) + prep_A + prep_B + prep_C + total_time + collab + num_subjects, data=x.new)
summary(m2)
```

We can see from this summary that the log transformation of the years_teacher predictor also produced an increase in the R-squared variable, moving it from 0.2464 to 0.2474.

Lastly, let's make sure that the assumptions for linear regression hold.
```{r}
...
*Note, I don't think it's worth doing logistic, since we can't show a bimodal distribution, but I can do this if we still want to include it*
...
```
```{r}
plot(teacher_data[teacher_data$region == "Western Europe",]$prep_A)
...
>>>>>>> bc65444179ed1550fdf775d8e0d663a57884007e
```
...
```
0.08257/0.2474 is roughly 33.3%, therefore the country variable alone accounts for 33% of the variability that our final model in Part 1 was able to explain.
