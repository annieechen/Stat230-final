...
```{r}
# first mosaic plot by region

teacher_data %>% filter(!is.na(collab)) %>% ggplot() + 
   geom_mosaic(aes(x = product(collab, region), fill=factor(collab)), na.rm=TRUE) + scale_fill_manual(values=c("red3","indianred1", "lightgreen", "green3")) +  theme(axis.text.x=element_text(angle=-25, hjust= .1)) + labs(x="Region", title='How Collaborative is School Culture') + guides(fill=guide_legend(title = "Response", reverse = TRUE))
