...
We can see that there are quite a few values of exactly 90, given the fact that the length of the vector where total_time is greater than or equal to 90 is so much longer than the vector that is strictly greater than 90.  Given this fact, we will keep the values of 90, but set all values that are higher than 90 hours a week to NA, since it seems highly unlikely that any teacher is working more than 90 hours a week.

```{r, echo=TRUE}
teacher_data[teacher_data$total_time < 90,]$total_time <- NA
```
...
nrow(teacher_data[age_start <= 15,])
teacher_data <- teacher_data[which(age_start >= 15),]
```

Now, because we can see from the above plot that age and years teaching are strongly positively correlated, we know that it doesn't make sense to include both of those as predictors. We can take a look at the histogram of the age_start variable - perhaps when you start teaching may affect how satisfied you are. Of course, if we interpret this variable directly as the age teachers start teaching, we are making an implicit assumption that they never took a break/time off. 

```{r}
as.data.frame(age_start) %>% ggplot(aes(x=age_start)) + geom_histogram(binwidth = 2) + labs(title="Histogram of Age When started teaching", x="Age (years) - Years working as a teacher", y="Count")
```

Now, let's add this column to our data frame, and remove the years_teaching variable

```{r}
teacher_data$years_teacher <- NULL
...
x.new <- teacher_data[, -c(sat_indices, na_cols, subject_indices)]
x.new <- na.omit(x.new[, -which(names(x.new) %in% c("train_stat", "region"))])
```
