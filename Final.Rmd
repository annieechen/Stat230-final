...
m1 <- lm(sat_avg ~ ., data=x[,-13])
#try model with region variable and not country
m2 <- lm(I(sat_avg^2) ~ ., data=x[,-13])
summary(m1)
summary(m2)
```
...
We can see from the results above, that all three stepwise models resulted in using the same 9 predictors, which were: collab, country, prep_A, prep_B, prep_C, gender, age, total_time, and years_teacher.
