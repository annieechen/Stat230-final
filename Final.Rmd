...
plot1 <- teacher_data %>% filter(!is.na(teacher_data$train_stat)) %>% ggplot(aes(x=train_stat, y=sat_avg)) + 
  geom_boxplot() +
  labs(title="Teacher Training Status vs Teacher Satisfaction", y="Teacher Satisfaction (low to high)") +
...
#code for map with average teacher satisfaction by country
sPDF2 <- joinCountryData2Map(country_region_pairs, joinCode = "ISO3", nameJoinColumn = "country")
...
potato <- do.call(addMapLegendBoxes, c(mapParams2, x='bottomleft',title="Region", cex=.5))
```
