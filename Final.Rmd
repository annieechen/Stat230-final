...
```{r}
# collab plot
p41 <- td_country %>% filter(!is.na(collab)) %>% ggplot(aes(x = product(collab, country), fill=factor(collab)), na.rm=TRUE) +  geom_mosaic() + scale_fill_manual(values=c("red3","indianred1", "lightgreen", "green3")) +  theme(axis.text.x=element_text(angle=-25, hjust= .1)) + labs(x="Country", title='How Collaborative is School Culture') + guides(fill=guide_legend(title = "Response", reverse = TRUE))
...
td_country %>%
    filter(country == "MEX") %>%
    ggplot(aes(x=collab, y=sat_avg)) + geom_boxplot()
    
    
    td_country %>% filter(country=="MEX") %>% ggplot(aes(x = product(sat10, collab), fill=factor(sat1)), na.rm=TRUE) +  geom_mosaic() + scale_fill_manual(values=c("red3","indianred1", "lightgreen", "green3")) +  theme(axis.text.x=element_text(angle=-25, hjust= .1)) + labs(x="Country", title='How Collaborative is School Culture') + guides(fill=guide_legend(title = "Response", reverse = TRUE))

    
plot(td_country[td_country$country == "MEX",]$collab, td_country[td_country$country == "MEX",]$sat_avg)
```
