...
plot2 <- teacher_data %>% ggplot(aes(x=years_teacher, y=sat_avg)) + geom_point(alpha=0.05) +
  ...
  theme(axis.text=element_text(size=8),
        axis.title=element_text(size=8),
        plot.title=element_text(size=8))
plot3 <- teacher_data %>% filter(!is.na(prep_A)) %>% ggplot(aes(x=prep_A, y=sat_avg)) + geom_boxplot() + labs(title="Preparation vs Satisfaction", x="Prepared for the Content I Teach", y="Teacher Satisfaction (low to high)") +
  theme(axis.text=element_text(size=8),
        axis.title=element_text(size=8),
        plot.title=element_text(size=8))
