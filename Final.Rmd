...
```
We did find that the difference in mean satisfaction was still statistically significant (p-vale < 0.05) across all region pairs other than Western Europe vs Eastern Europe, as shown below.
...
# reorder country factors from least to most satisfied
```

Based on which predictors affected job satisfaction in Part 1, we can see which countries stand out, either positively or negatively, in those factors 
...
p42 <- td_country %>% filter(!is.na(avg_prep)) %>% ggplot(aes(x=reorder(region, -avg_prep, median), y=avg_prep)) + geom_boxplot() + xlab("Region") + ylab("Average Feeling of Preparation")
