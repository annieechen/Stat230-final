...
# prep plot
p42 <- teacher_data[, c("prep_A", "prep_B", "prep_C","region")] %>%
...
# gender plot
p43 <- teacher_data %>% filter(!is.na(gender)) %>% ggplot(aes(x = product(gender, region), fill=factor(gender)), na.rm=TRUE) +  geom_mosaic()  +  theme(axis.text.x=element_text(angle=-25, hjust= .1)) + labs(x="Region", title='Gender') + guides(fill=guide_legend(title = "Response", reverse = TRUE))

# age plot
p44 <- teacher_data %>% filter(!is.na(age)) %>% ggplot(aes(x=reorder(region, -as.numeric(age), median), y=as.numeric(age))) + geom_boxplot() + xlab("Region") + ylab("Age")

# total_time plot
p45 <- teacher_data %>% filter(!is.na(total_time)) %>% ggplot(aes(x=reorder(region, -as.numeric(total_time), median), y=as.numeric(total_time))) + geom_boxplot() + xlab("Region") + ylab("Total Time Working")

# age_start plot
p46 <- teacher_data %>% filter(!is.na(age_start)) %>% ggplot(aes(x=reorder(region, -as.numeric(age_start), median), y=as.numeric(age_start))) + geom_boxplot() + xlab("Region") + ylab("Years not spent teaching")
```

```{r, echo=FALSE, fig.height=10, fig.width=10, message=FALSE, warning=FALSE}
grid.arrange(p41, p42, p43, p44, p45, p46, ncol=2)
```
...
# age and region LM
lmagereg <- lm(sat_avg ~ region*as.numeric(age), data=teacher_data)

# total_time and region LM
lmttreg <- lm(sat_avg ~ region*as.numeric(total_time), data=teacher_data)

# years_working and region LM
lmywreg <- lm(sat_avg ~ region*as.numeric(years_teacher), data=teacher_data)
```
